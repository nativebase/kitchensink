import React from "react";
import { createStackNavigator } from "@react-navigation/stack";
import {
  PinInputDefaultValue,
  PinInputFormControlled,
  PinInputManageFocus,
  PinInputPlaceholder,
  PinInputSize,
  PinInputUsage,
  PinInputVariant,
} from "../screens";
import { MenuButton } from "../components";

const Stack = createStackNavigator();
export function PinInputStack(props: any) {
  return (
    <Stack.Navigator initialRouteName="pinInputUsage">
      <Stack.Screen
        name="pinInputUsage"
        component={PinInputUsage}
        options={{
          headerTitle: "PinInput Usage",
          headerLeft: () => {
            return <MenuButton toggleDrawer={props.navigation.toggleDrawer} />;
          },
        }}
      />
      <Stack.Screen
        name="pinInputSize"
        component={PinInputSize}
        options={{
          headerTitle: "PinInput Size",
          headerLeft: () => {
            return <MenuButton toggleDrawer={props.navigation.toggleDrawer} />;
          },
        }}
      />
      <Stack.Screen
        name="pinInputDefaultValue"
        component={PinInputDefaultValue}
        options={{
          headerTitle: "PinInput DefaultValue",
          headerLeft: () => {
            return <MenuButton toggleDrawer={props.navigation.toggleDrawer} />;
          },
        }}
      />
      <Stack.Screen
        name="pinInputPlaceholder"
        component={PinInputPlaceholder}
        options={{
          headerTitle: "PinInput Placeholder",
          headerLeft: () => {
            return <MenuButton toggleDrawer={props.navigation.toggleDrawer} />;
          },
        }}
      />
      <Stack.Screen
        name="pinInputManageFocus"
        component={PinInputManageFocus}
        options={{
          headerTitle: "PinInput Manage Focus",
          headerLeft: () => {
            return <MenuButton toggleDrawer={props.navigation.toggleDrawer} />;
          },
        }}
      />
      <Stack.Screen
        name="pinInputVariant"
        component={PinInputVariant}
        options={{
          headerTitle: "PinInput Variants",
          headerLeft: () => {
            return <MenuButton toggleDrawer={props.navigation.toggleDrawer} />;
          },
        }}
      />
      <Stack.Screen
        name="pinInputFormControlled"
        component={PinInputFormControlled}
        options={{
          headerTitle: "PinInput FormControlled",
          headerLeft: () => {
            return <MenuButton toggleDrawer={props.navigation.toggleDrawer} />;
          },
        }}
      />
    </Stack.Navigator>
  );
}
